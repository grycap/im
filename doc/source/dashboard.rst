...
* Add nodes: The Add nodes action enables to add new VMs to the users' deployment. As depicted in
  ...
  will also show the image selected to create the VMs (if available). In the dropdows list the user can 
  change the base image in case that you want to use a new one on the new deployed nodes or that old one
  has been removed and you need to select a new one to be able to deploy new nodes.
