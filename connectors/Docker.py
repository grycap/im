...
class DockerCloudConnector(CloudConnector):
	...
	def launch(self, radl, requested_radl, num_vm, auth_data):
		...
		create_request_json = """ {
			 ...
			 %s
		}""" % (memory,image_name,volumes)
		
		

		
		
		# TODO: este puerto hay que generarlo dinamicamente y asegurarse de que no esta ocupado 
		# en la maquina servidor de Docker
		# luego habra que devolvelo en el RADL
		ssh_port = 11022
		start_request_json = """ {
		 "LxcConf":[{"Key":"lxc.utsname","Value":"docker"}],
		 "PortBindings":{ "22/tcp": [{ "HostPort": "%s" }] },
