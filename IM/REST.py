...
service_ns = flask_restx.Namespace('service', description='General Service calls.', path='/')
api.add_namespace(infra_ns)
...
@cloud_ns.route('/<string:cloudid>/<string:param>')
class CloudInfo(flask_restx.Resource):
        ...
        try:
            if param == 'images':
                ...
                if "filters" in flask.request.args.keys():
                    try:
                        filters = self._filters_str_to_dict(flask.request.args.get("filters"))
                    except Exception:
