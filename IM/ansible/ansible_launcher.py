...
class AnsibleThread(threading.Thread):
    ...
    def launch_playbook_v2(self):
        ...
        while return_code != 0 and num_retries < self.retries:
            ...
            if return_code != 0:
                display("ERROR executing playbook (%s/%s)" % (num_retries, self.retries),  output=self.output)
    ...
    def launch_playbook_v1(self):
        ...
        while return_code != 0 and num_retries < self.retries:
            ...
            playbook_cb = PlaybookCallbacks(verbose=utils.VERBOSITY, output=self.output)
            runner_cb = PlaybookRunnerCallbacks(stats, verbose=utils.VERBOSITY, output=self.output)
            ...
            if return_code != 0:
                display("ERROR executing playbook (%s/%s)" % (num_retries, self.retries), color='red', output=self.output)
