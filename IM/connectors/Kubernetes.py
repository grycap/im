...
class KubernetesCloudConnector(CloudConnector):
    ...
    def create_request(self, method, url, auth_data, headers=None, body=None):
        ...
        if body and isinstance(body, (dict, list)):
            ...
            data = body
        url = "%s://%s:%d%s%s" % (self.cloud.protocol, self.cloud.server, self.cloud.get_port(),
                                  self.cloud.path.rstrip('/'), url)
                ...
                if 'persistentVolumeClaim' in volume and 'claimName' in volume['persistentVolumeClaim']:
                    ...
                    if vc_name not in managed_pvcs:
                        self.log_debug("Not deleting external PVC: %s/%s" %
                                       (dep_data["metadata"]["namespace"], vc_name))
                        continue
    ...
    def _generate_dep_data(self, namespace, name, outports, system, volumes, configmaps, tags):
        ...
        if volumes:
            ...
            if managed_pvcs:
                annotations = {self.MANAGED_PVCS_ANNOTATION: ','.join(managed_pvcs)}
                dep_data['metadata']['annotations'] = annotations
                dep_data['spec']['template']['metadata']['annotations'] = annotations
